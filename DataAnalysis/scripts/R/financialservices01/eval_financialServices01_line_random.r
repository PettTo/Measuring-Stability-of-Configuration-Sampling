...
placeholder <- c(rep(NaN,length(roicRandom)))

### preparation x and y
versionX <- c(1:length(icstRandom)-1)
x <- c(1:length(icstRandom)-1)
y <- c(1:length(icstRandom)-1)
...
pdf(file='./plots/financialServices01_line_random.pdf', width=9, height=6)
plot(x,y, las=2, ylim=c(0,1),main="FinancialServices01 Random Line plot",
      axes=FALSE, type="n"
)
lines(
  versionX,roicRandom,
...
#     labels=c("","Random","Chvatal","ICPL","Random",
#              "","Random","Chvatal","ICPL","Random",
#             "","Random","Chvatal","ICPL","Random",
#              ""), 
     las=2)
axis(2, 
