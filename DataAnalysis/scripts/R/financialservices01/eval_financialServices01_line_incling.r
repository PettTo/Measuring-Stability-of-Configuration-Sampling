...
### preparation x and y
versionX <- c(1:length(icstIncling)-1)
x <- c(1:length(icstIncling)-1)
y <- c(1:length(icstIncling)-1)
...
pdf(file='./plots/financialServices01_line_incling.pdf', width=9, height=6)
plot(x,y, las=2, ylim=c(0,1),main="FinancialServices01 Incling Line plot",
      axes=FALSE, type="n"
)
lines(
  versionX,roicIncling,
  type="b",
  lty=1,
  lwd=2,
  pch=16,
