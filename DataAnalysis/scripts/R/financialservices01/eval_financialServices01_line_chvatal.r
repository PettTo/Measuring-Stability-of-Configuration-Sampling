...
placeholder <- c(rep(NaN,length(roicChvatal)))

### preparation x and y
versionX <- c(1:length(icstChvatal)-1)
x <- c(1:length(icstChvatal)-1)
y <- c(1:length(icstChvatal)-1)
...
pdf(file='./plots/financialServices01_line_chvatal.pdf', width=9, height=6)
plot(x,y, las=2, ylim=c(0,1),main="FinancialServices01 Chvatal Line plot",
      axes=FALSE, type="n"
)
lines(
  versionX,roicChvatal,
  type="b",
  lty=1,
  lwd=2,
  pch=16,
...
#     labels=c("","Random","Chvatal","ICPL","Chvatal",
#              "","Random","Chvatal","ICPL","Chvatal",
#             "","Random","Chvatal","ICPL","Chvatal",
#              ""), 
     las=2)
axis(2, 
